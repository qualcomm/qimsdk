...
*
* Usage:
* gst-camera-per-port-example [-custom | -default [-c NUM_CAMERAS] [-s NUM_STREAMS] [--preview]]
*
*/
#include <iostream>
#include <sstream>
...
// Preview tile dimensions for waylandsink render-rectangle positioning
#define PREVIEW_TILE_W  640
#define PREVIEW_TILE_H  480
...
struct _GstPerPortCtx
{
  ...
  gboolean reconfig;
  // Tile index for waylandsink render-rectangle (preview mode)
  gint tile_index;
};
...
handle_interrupt_signal (gpointer userdata)
{
  ...
  for (; i < appctx->ctx.size (); ++i) {
    if (appctx->ctx[i].pipeline == NULL) continue;
    ret = gst_element_get_state (appctx->ctx[i].pipeline, &state, &pending,
        GST_CLOCK_TIME_NONE);
    if (ret == GST_STATE_CHANGE_FAILURE) {
      g_print ("ERROR: %s get current state! %d\n", appctx->ctx[i].pipe_name, ret);
      continue;
    }
    if (state == GST_STATE_PLAYING) {
      if (appctx->preview_mode) {
        // waylandsink does not generate EOS - go directly to NULL
        gst_element_set_state (appctx->ctx[i].pipeline, GST_STATE_NULL);
      } else {
        gst_element_send_event (appctx->ctx[i].pipeline, gst_event_new_eos ());
      }
    }
  }
  if (appctx->preview_mode) {
    g_main_loop_quit (appctx->ctx[0].mloop);
  }
  return TRUE;
}
// Handle warnings
...
// End of stream callback function (used only in record/multifilesink mode)
static void
    ...
    gboolean preview_mode)
{
  ...
  // Show current mode in the menu header
  if (preview_mode) {
    g_string_append_printf (options,
        "   Mode: PREVIEW (waylandsink + qtivtransform)\n\n");
  } else {
    g_string_append_printf (options,
        "   Mode: RECORD (multifilesink)\n\n");
  }
  APPEND_CAMERA_ON_OFF_CONTROLS_SECTION (options);
  // Active cameras (can be stopped)
  for (auto & item:ctx) {
    if (item.pipeline == NULL) continue;
    if (item.active) {
      std::string cam_id = std::to_string (item.camera);
      g_string_append_printf (options, "   (%s) : Stop camera %s\n",
          cam_id.c_str (), cam_id.c_str ());
    }
  }
  // Inactive cameras (can be started)
  for (auto & item:ctx) {
    if (item.pipeline == NULL) continue;
    if (!item.active) {
      std::string cam_id = std::to_string (item.camera);
      g_string_append_printf (options, "   (%s) : Start camera %s\n",
          cam_id.c_str (), cam_id.c_str ());
    }
  }
  g_string_append_printf (options, "   (%s) : Exit the application\n", "q");
  g_print ("%s", options->str);
  g_string_free (options, TRUE);
}
static gboolean
gst_active_cameras_menu (std::vector < GstPerPortCtx > &ctx,
    GAsyncQueue * messages, gboolean preview_mode)
{
  gchar *input = NULL;
  ...
  GstState state, pending;
  GstStateChangeReturn ret;
  print_active_camera_options (ctx, preview_mode);
  g_print ("\n\nChoose an Option : ");
  ...
  if (g_str_equal (input, QUIT_OPTION)) {
    ...
    for (auto & item:ctx) {
      if (item.pipeline == NULL) continue;
      if (preview_mode) {
        // waylandsink does not generate EOS - tear down directly
    ...
    // For preview mode quit the main loop directly since no EOS will arrive
    if (preview_mode) {
      g_main_loop_quit (ctx[0].mloop);
    }
  } else {
...
main_menu (gpointer userdata)
{
  GstAppContext *appctx = GST_APP_CONTEXT_CAST (userdata);
  gboolean active = TRUE;
  // Work on the shared ctx vector so stop/start changes are visible
  while (active) {
    active = gst_active_cameras_menu (appctx->ctx, appctx->messages,
        appctx->preview_mode);
  }
  return NULL;
}
...
main (gint argc, gchar * argv[])
{
  ...
  gboolean preview_mode = FALSE;   // NEW: --preview flag
  GOptionEntry entries[] = {
    ...
    { "preview", 0, 0, G_OPTION_ARG_NONE,
      &preview_mode,
      "Use waylandsink (via qtivtransform) for live preview instead of multifilesink", NULL
    },
  ...
  // --preview is only meaningful with --default
  if (preview_mode && !default_mode) {
    g_printerr ("ERROR: --preview flag requires --default mode.\n");
    g_option_context_free (context);
    return 1;
  }
  if (!custom_mode && !default_mode) {
  ...
  for (guint i = 0; i < camInfo.size (); ++i) {
    ...
    camctx.tile_index = (gint)i;   // used for waylandsink positioning
    ...
    if (custom_mode) {
      ...
      if (preview_mode) {
        ...
        // PREVIEW pipeline:
        //
        //   qtiqmmfsrc camera=N video_0::type=preview
        //     ! video/x-raw,format=NV12_Q08C,width=W,height=H,framerate=F/1
        //     ! qtivtransform
        //         scale-width=PREVIEW_TILE_W scale-height=PREVIEW_TILE_H
        //     ! video/x-raw,format=NV12,width=PREVIEW_TILE_W,height=PREVIEW_TILE_H
        //     ! waylandsink sync=false
        //         render-rectangle="<tile_x,0,PREVIEW_TILE_W,PREVIEW_TILE_H>"
        //
        //  Each camera occupies its own horizontal tile so no overlapping:
        //    Camera 0 -> x=0,    Camera 1 -> x=640,  Camera 2 -> x=1280 ...
        // ---------------------------------------------------------------
        gint tile_x = (gint)i * PREVIEW_TILE_W;
        std::string caps_src = "video/x-raw,format=NV12_Q08C";
        caps_src += ",width="     + std::to_string (camInfo[i].streamconfig[0].width);
        caps_src += ",height="    + std::to_string (camInfo[i].streamconfig[0].height);
        caps_src += ",framerate=" + std::to_string (camInfo[i].streamconfig[0].framerate) + "/1";
        std::string caps_xform = "video/x-raw,format=NV12";
        caps_xform += ",width="  + std::to_string (PREVIEW_TILE_W);
        caps_xform += ",height=" + std::to_string (PREVIEW_TILE_H);
        std::string rect = std::to_string (tile_x)        + ","
                         + std::to_string (0)              + ","
                         + std::to_string (PREVIEW_TILE_W) + ","
                         + std::to_string (PREVIEW_TILE_H);
        cam_pipeline =
            "qtiqmmfsrc name=camsrc" + std::to_string (camInfo[i].camera_id) +
            " camera=" + std::to_string (camInfo[i].camera_id) +
            " video_0::type=preview ! " + caps_src +
            " ! qtivtransform ! " + caps_xform +
            " ! gtkwaylandsink sync=false ";
        g_print ("Generated PREVIEW pipeline for camera %u: %s\n",
            camInfo[i].camera_id, cam_pipeline.c_str ());
      } else {
        // ---------------------------------------------------------------
        // RECORD pipelines (original multifilesink logic)
        // ---------------------------------------------------------------
        if (camInfo[i].streamconfig.size () == 1) {
          ...
          caps_str += ",framerate=" + std::to_string (camInfo[i].streamconfig[0].framerate) + "/1";
          cam_pipeline =
              "qtiqmmfsrc name=camsrc" + std::to_string (camInfo[i].camera_id) +
              " camera=" + std::to_string (camInfo[i].camera_id) +
              " video_0::type=video ! " + caps_str +
              " ! multifilesink location=/data/frame" +
          ...
          caps_str_0 +=
              ",framerate=" + std::to_string (camInfo[i].streamconfig[0].framerate) + "/1";
          stream_links += " camsrc.video_0 ! " + caps_str_0 +
              " ! multifilesink location=/data/frame" +
              std::to_string (camInfo[i].camera_id) +
              "_stream0_%d.yuv sync=true max-files=2 ";
          // Stream 1: format NV12
          ...
          caps_str_1 +=
              ",framerate=" + std::to_string (camInfo[i].streamconfig[1].framerate) + "/1";
          stream_links += " camsrc. ! " + caps_str_1 +
              " ! multifilesink location=/data/frame" +
              std::to_string (camInfo[i].camera_id) +
              "_stream1_%d.yuv sync=true max-files=2";
          cam_pipeline =
              "qtiqmmfsrc name=camsrc camera=" + std::to_string (camInfo[i].camera_id) +
  ...
  appctx.ctx          = ctx;
  appctx.preview_mode = preview_mode;
  appctx.messages     =
  ...
  for (guint i = 0; i < appctx.ctx.size (); i++) {
    ...
    g_signal_connect (bus, "message::error", G_CALLBACK (error_cb),
        &appctx.ctx[i]);
    // EOS callback only needed for record mode (waylandsink never emits EOS)
    if (!preview_mode) {

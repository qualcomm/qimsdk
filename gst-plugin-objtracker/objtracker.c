  ...
  switch (prop_id) {
    ...
    case PROP_PARAMETERS:
    {
      ...
      if (g_file_test (string, G_FILE_TEST_IS_REGULAR) &&
        ...
        GST_ERROR_OBJECT (objtracker, "Failed to deserialize file!");
        break;
      } else if (!gst_value_deserialize (&structure, string)) {
        GST_ERROR_OBJECT (objtracker, "Failed to deserialize string!");
        break;
      }

      g_clear_pointer (&objtracker->algoparameters, gst_structure_free);
      objtracker->algoparameters = GST_STRUCTURE (g_value_dup_boxed (&structure));

...
class MLVideoONNXBin(Element):
    """Wrapper for qtimlvideoonnxbin that exposes preprocess and postprocess handler functionality."""

    def __init__(self, name: str | None = None, *, existing: Any | None = None) -> None:
        super().__init__("qtimlvideoonnxbin" if existing is None else None, name, existing=existing)

        self._postprocess: Optional[MLPostprocess] = None
        self._preprocess: Optional[MLVConverter] = None

    @classmethod
    def cast(cls, element: Any) -> "MLVideoONNXBin":
    ...
    def set_postprocess_handler(self, handler: Any, kind: str | None = None) -> "MLVideoONNXBin":
        post = self._ensure_postprocess()
        post.set_handler(handler, kind)
        return self

    def set_preprocess_handler(self, handler: Any) -> "MLVideoONNXBin":

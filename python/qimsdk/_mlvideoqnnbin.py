...
class MLVideoQNNBin(Element):
    """Wrapper for qtimlvideoqnnbin that exposes preprocess and postprocess handler functionality."""

    def __init__(self, name: str | None = None, *, existing: Any | None = None) -> None:
        super().__init__("qtimlvideoqnnbin" if existing is None else None, name, existing=existing)

        self._postprocess: Optional[MLPostprocess] = None
        self._preprocess: Optional[MLVConverter] = None

    @classmethod
    def cast(cls, element: Any) -> "MLVideoQNNBin":
    ...
    def set_postprocess_handler(self, handler: Any, kind: str | None = None) -> "MLVideoQNNBin":
        post = self._ensure_postprocess()
        post.set_handler(handler, kind)
        return self

    def set_preprocess_handler(self, handler: Any) -> "MLVideoQNNBin":

...
class MLVideoTFLiteBin(Element):
    """Wrapper for qtimlvideotflitebin that exposes preprocess and postprocess handler functionality."""

    def __init__(self, name: str | None = None, *, existing: Any | None = None) -> None:
        super().__init__("qtimlvideotflitebin" if existing is None else None, name, existing=existing)

        self._postprocess: Optional[MLPostprocess] = None
        self._preprocess: Optional[MLVConverter] = None

    @classmethod
    def cast(cls, element: Any) -> "MLVideoTFLiteBin":
    ...
    def _ensure_postprocess(self) -> MLPostprocess:
        """Ensure the postprocess wrapper is initialized."""
    ...
    def _ensure_preprocess(self) -> MLVConverter:
        """Ensure the preprocess wrapper is initialized."""
    ...
    def set_postprocess_handler(self, handler: Any, kind: str | None = None) -> "MLVideoTFLiteBin":
        """Delegate postprocess handler setup to internal MLPostprocess."""
        post = self._ensure_postprocess()
        post.set_handler(handler, kind)
        return self

    def set_preprocess_handler(self, handler: Any) -> "MLVideoTFLiteBin":

...
"""Custom palm-detection preprocess example (native palmd postprocess)."""
...
from qimsdk import CamSrc, Element, Pipeline, VideoFilter, TextFilter, MLVConverter
...
#
#  The pipeline captures camera frames, runs palm detection with an external
#  Python preprocessing callback feeding the native postprocessing element,
#  overlays the detected palms, and displays the result through Wayland.
...
def convert_nv12_to_rgb(planes: List[memoryview], info, destination, tensor: np.ndarray) -> bool:
    ...
    # outside the [0, 1] range the model expects.
    if tensor.dtype != np.float32:
        print(f"preprocess failed: expected float32 tensor | dtype={tensor.dtype}")
        return False
...
def preprocess_callback(blits, output) -> bool:
    print(
        f"[external-preprocess][palm-detect] called: "
...
def create_and_execute_pipeline() -> None:
    ...
    # Restricts the camera stream to NV12/1080p/30fps before it reaches the tee.
    ...
    pipeline = (
        Pipeline("ml-external-preprocess-palm-detect")
        .add(source)
